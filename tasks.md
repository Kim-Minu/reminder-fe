...
### 예산 (`/budget`)
- [x] `app/budget/page.tsx` 생성
- [x] 예산 타입 정의 (`category`, `amount`, `spent`, `month`)
- [x] 이번 달 총 예산 / 지출 / 잔액 요약 카드
- [x] 카테고리별 예산 목록 (프로그레스 바 — 지출 비율)
- [x] 예산 항목 추가 / 수정 / 삭제
- [x] 월 선택 네비게이션 (이전/다음 달)
- [x] API 연동 또는 로컬 스토어 결정
